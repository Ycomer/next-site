...
`<title>` is part of the `<head>` HTML tag of a page. Let's dive into how we can modify the `<head>` of pages.
...
We haven't added a `<title>` to our `/posts-first-page` route. Let's add one.

Open the `pages/posts/first-page.js` file.
...
>
> If you want to customize the `<html>`, for example to add the `lang` attribute, you can do so by creating a custom `Document` component. Learn more on <Link href="/docs/[...slug]" as="/docs/advanced-features/custom-document"><a>the custom document documentation</a></Link>.
