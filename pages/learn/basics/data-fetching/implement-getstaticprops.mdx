...
Next, we’ll create a simple library for fetching data from the filesystem.
...
You should now see the blog data if you access [localhost:3000](http://localhost:3000).
...
Congratulations! We successfully fetched external data (from the file system) and pre-rendered the index page with this data.
...
Before we wrap up this lesson, let’s talk about some tips for using `getStaticProps` on the next page.
