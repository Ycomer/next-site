...
To link to a page with dynamic routes, you need to use the `Link` component differently. In our case, to link to `/posts/dps`, you need to write it like this:

```jsx
<Link href="/posts/[id]" as="/posts/dps">
  <a>...</a>
</Link>
```

As you can see, you need to use `[id]` for `href` and the actual path (`dps`) for the `as` prop.

Let’s implement it. First, import `Link` and `Date` in `posts/index.js`:
